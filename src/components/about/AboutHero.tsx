...
export function AboutHero() {
  ...
  useLayoutEffect(() => {
    ...
    const subLines = Array.from(el.querySelectorAll<HTMLElement>('.hero-sub-line'))
    const grid = el.querySelector<HTMLElement>('.pg-hero-grid')
    const o1 = el.querySelector<HTMLElement>('#aboutHeroO1')
    const o2 = el.querySelector<HTMLElement>('#aboutHeroO2')
    const o3 = el.querySelector<HTMLElement>('#aboutHeroO3')

    const reduced = prefersReducedMotion()

    let tl: gsap.core.Timeline | null = null
    if (!reduced && chip && words.length && subLines.length) {
      gsap.set(chip, { opacity: 0, scale: 0.9, filter: 'blur(8px)', y: -6 })
      gsap.set(words, { y: '110%' })
      gsap.set(subLines, { opacity: 0, y: 18, filter: 'blur(10px)' })

      tl = gsap.timeline({ defaults: { ease: 'expo.out' } })
      tl.to(chip, { opacity: 1, scale: 1, filter: 'blur(0px)', y: 0, duration: 0.8 }, 0.15)
        .to(words, { y: '0%', duration: 1, stagger: 0.13 }, 0.35)
        .to(subLines, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.82, stagger: 0.14 }, 0.68)
    } else if (reduced && chip) {
      gsap.set(chip, { opacity: 1, scale: 1, filter: 'blur(0px)', y: 0 })
      gsap.set(words, { y: '0%' })
      gsap.set(subLines, { opacity: 1, y: 0, filter: 'blur(0px)' })
    }
  ...
  return (
    <header ref={heroRef} className="pg-hero" aria-labelledby="about-hero-heading">
      <div className="pg-hero-bg" aria-hidden>
        <div className="pg-hero-noise" />
        <div className="pg-hero-grid" />
        <div className="pg-hero-orb pg-hero-orb--a" id="aboutHeroO1" />
        <div className="pg-hero-orb pg-hero-orb--b" id="aboutHeroO2" />
        <div className="pg-hero-orb pg-hero-orb--c" id="aboutHeroO3" />
      </div>

      <ParticlesBackground id="about-hero-particles" />
      ...
      <div className="pg-hero-inner">
        ...
        <h1 id="about-hero-heading">
          <span className="h1-clip">
            <span className="h1-word">About</span>
          </span>{' '}
          <span className="h1-clip">
            <span className="h1-word gradient-text">VokDev</span>
          </span>
        </h1>

        <p className="hero-sub max-w-3xl">
          <span className="hero-sub-line block">
            A global home for developers, designers, and builders who ship real work, learn faster,
            and grow alongside peers — not in isolation.
          </span>
          <span className="hero-sub-line mt-2 block text-muted-foreground">
            Collaboration, craft, and community drive everything we do.
          </span>
        </p>
      </div>
    </header>
