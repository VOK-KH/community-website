...
import { authUserDisplay } from './user-display'
...
/** Sidebar / account row: friendly name, email, initials, and optional profile image. */
export function getCmsSessionDisplay(session: CmsSession) {
  return authUserDisplay(session.user)
}
